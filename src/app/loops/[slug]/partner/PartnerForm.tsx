...
export default function PartnerForm({ slug, brandName }: { slug: string; brandName: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/partner-apply', {
      ...
      body: JSON.stringify({ competition: slug, handle, name, specialty, role, reach, platform, portfolioUrl: portfolio }),
    })
    if (res.ok) {
      setDone(true)
    } else {
  ...
  return (
    <div className="portal">
      <header className="portal-header">
        ...
        <div className="phase">Join the Partner Roster</div>
      </header>
      ...
      {done ? (
        <div style={{ textAlign: 'center', padding: '32px 0' }}>
          ...
          <div style={{ fontSize: 14, color: '#555', lineHeight: 1.6, marginBottom: 24 }}>
            The {brandName} team will review your profile. If there&apos;s a fit, they&apos;ll reach out directly.
          </div>
        ...
        <form onSubmit={submit}>
          <p style={{ fontSize: 14, color: '#555', lineHeight: 1.6, marginBottom: 20 }}>
            Raise your hand to amplify the {brandName} competition. The brand can browse and reach out directly.
          </p>
          ...
          <div className="form-section">
            ...
            <input id="name" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Jamie O'Brien" />
          </div>

          <div className="form-section">
            <label className="form-label" htmlFor="specialty">
              Specialty
              <span className="hint">e.g. Activewear Content, TikTok Growth</span>
            </label>
            <input id="specialty" value={specialty} onChange={e => setSpecialty(e.target.value)} placeholder="What you do best" />
          </div>
